...
const ip = new window.PI(JSON.parse(JSON.stringify(page)), null, { user: { nick: 11 } });
if (!~location.search.indexOf('from=ide')) {

  setTimeout(() => {
    page.components[0].props.dataSource = [{ text: '测试 reset' }];
    ip.refresh(JSON.parse(JSON.stringify(page)));
  }, 3000);
}
...
const handleFrameTasks = message => {
  console.log('message event:', message.event);
  if (message.type === 'updateSchema') {
    ip.refresh(JSON.parse(JSON.stringify(message.data)));
  }
};
...
window.addEventListener('message', e => {
  const message = parseOrFalse(e.data);
  const allows = [].concat('data-from-ide');
  if (!!message && allows.includes(message.event)) {
